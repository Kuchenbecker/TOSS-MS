...
def plot_msms_with_labels(mzml_file, scan_num=None, title=None, intype='r',
                         ...
                         overlay_files=None, three_d=False):
    
    if overlay_files and three_d:
        # 3D plot mode
        files_data = []
        
        # Get HCD energy for main file from filename
        main_hcd = get_hcd_energy_from_filename(mzml_file)
        if main_hcd is None:
            print(f"Warning: Could not determine HCD energy from filename {mzml_file}")
        
        # Process the main file
        mz_array, intensity_array, scan_label = get_spectrum_data(mzml_file, scan_num)
        ...
        files_data.append({
            ...
            'hcd_energy': main_hcd,
            'label': os.path.basename(mzml_file)
        })
        ...
        for overlay_file in overlay_files:
            try:
                ...
                overlay_mz, overlay_intensity, _ = get_spectrum_data(overlay_file, scan_num)
        ...
        # 2D overlay mode (previous implementation)
        # ... [rest of the 2D overlay code remains the same] ...
        pass
    
    else:
        # Single file mode (original functionality)
        # ... [rest of the single file code remains the same] ...
        pass
...
if __name__ == "__main__":
    ...
    plot_msms_with_labels(
        ...
        args.labelmin,
        overlay_files,
        args.__dict__.get('3d', False)  # Handle the --3d flag
    )