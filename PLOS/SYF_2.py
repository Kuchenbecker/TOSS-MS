...
def save_tog_plot(df, energy_label, ions_to_plot, y_label_suffix=''):
    """Save a plot with only the specified ions"""
    plt.figure(figsize=(10, 6))
    ...
    # Format title with subscript for COM
    if energy_label == 'CECOM':
        title = r'Selected Ion Intensities vs. CE$_{\mathrm{COM}}$'
    else:
        title = f'Selected Ion Intensities vs. {energy_label}'
    ...
    # Save the plot
    plot_filename = f'ion_intensities_selected_{energy_label.lower()}{y_label_suffix.replace(" ", "_").lower()}.png'
    plt.savefig(plot_filename, dpi=300, bbox_inches='tight')
    print(f"Selected ions plot saved to {plot_filename}")
    plt.close()
...
def main():
    ...
    parser.add_argument('--tog', help='List of ions to plot together (comma-separated)')
    ...
    if args.plot:
        ...
        # Save selected ions plot if requested
        if args.tog:
            try:
                ions_to_plot = [str(float(mass.strip())) for mass in args.tog.split(',')]
                save_tog_plot(plot_df, energy_label, ions_to_plot, y_label_suffix)
            except ValueError:
                print("Warning: Invalid ion masses in --tog argument. Skipping selected ions plot.")
